...
- **修复 README 状态过期**：将当前状态从 MVP 0.1 更新为 MVP 0.1+，把 `swift test` 验证数从 44 个测试同步为 64 个测试，并补充 AI 时间结构分析、稿件导出、长稿导航、书签、倒计时、镜像和行距 / 字距等已实现能力。
- **同步英文 README**：保持 `README.en.md` 与中文主页一致，避免公开仓库中英文状态不一致。
- **更新 context.md**：把项目阶段从“需求定义 / 原型方向”调整为“本地可运行闭环完成，重点转向真实录制验收和 0.2 取舍”，并补充当前 SwiftPM + SwiftUI + AppKit、DeepSeek 两阶段 AI 管线的技术现状。
- **补齐代码型项目架构规则入口**：新增 `.claude/rules/architecture.md`，记录当前 `SottoCore` 纯逻辑层与 `Sotto` UI / 窗口层的边界，后续代码协作可按项目规则直接读取。
- **验证**：`swift test` 64 个测试通过。真实 3-5 分钟录制、麦克风首启、屏幕共享隐藏和长稿性能仍需单独验收。
...
- **新增"关于作者"章节**：以 Dv. (Dewens) 身份、幽默真诚的语气自我介绍——"不会写 Swift 的 AI 产品经理"、与 AI 结对编程的诚实过程，并留下 GitHub Issues 互动入口。
- **新增"不只是代码"章节**：强调仓库放的是"怎么从零做出来的"而不仅是源码，链接产品定义、视觉基线、动效原则、决策记录、逐日推进和真实验收 6 份过程文档，作为 vibe coding 完整过程的公开答卷。
- **设置仓库简介**：GitHub 仓库 description 设为"安静的演出后台 — 不会写 Swift 的 AI 产品经理用 vibe coding 做的 macOS 提词器"。
- **删除"给后续协作者"章节**：与新段落内容重叠，移除避免冗余。

## 2026-05-11（GitHub 项目主页中文化）

- **调整 GitHub 项目主页语言策略**：将 `README.md` 改为中文主版，让仓库首页默认面向中文读者；新增 `README.en.md` 保留英文版，并在两个 README 顶部增加「中文 / English」切换入口。
- **保留公开表达完整性**：中文主页覆盖项目定位、当前状态、核心功能、截图、本地运行、项目结构、路线图、已知限制、设计说明、公开发布记录、引用和授权说明；英文版基于原公开 README 保留完整内容。
