...
Sotto 目前是一个供个人使用和公开学习的 MVP 0.1+ 版本。核心提词闭环已经可以在本地运行，并已补入 AI 时间结构分析、稿件导出、长稿导航和现场提词增强；但它还不是一个完整打磨过的正式产品。

最近本地已验证：

- `swift test` 通过 64 个测试。
...
- 粘贴准备好的口播稿。
- 自动把中英文内容拆成句子和短语。
- 可选用 DeepSeek 进行两阶段 AI 分析：先提取纯净口播稿，再估算时间结构。
- 直接编辑当前句子。
- 把一句拆成两句，或与上一句 / 下一句合并。
- 为句子设置停顿和强调，影响提词节奏。
- 将稿件保存到本地稿件库。
...
- 点击句子即可在排练或录制中跳转。
- 通过 TUNE 抽屉调整速度、字号、透明度、宽度、亮度、行间距、字间距和镜像。
- 支持位置预设、书签、倒计时、长稿导航和常用键盘快捷键。
